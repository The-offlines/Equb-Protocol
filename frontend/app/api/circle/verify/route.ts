...
export async function POST(request: Request) {
  try {
    ...
    const wallets = (walletsResponse.data?.wallets ?? []) as CircleWallet[];
    ...
    if (arcWallet?.address) {
      return NextResponse.json({ walletAddress: arcWallet.address, walletId: arcWallet.id, userToken, encryptionKey, userId });
    }
    ...
    if (wallets.length > 0 && !challengeId) {
      ...
      if (newChallengeId) {
        return NextResponse.json({ walletAddress: null, challengeId: newChallengeId, userToken, encryptionKey, userId });
      }
      // If wallet creation fails, fall through to use existing wallet
      const anyWallet = wallets.find((w) => w.address);
      if (anyWallet?.address) {
        return NextResponse.json({ walletAddress: anyWallet.address, walletId: anyWallet.id });
      }
    }
    ...
    if (!challengeId) {
      try {
        const pinChallenge = await client.createUserPinWithWallets({
          ...
          blockchains: ["ARC-TESTNET"],
          idempotencyKey: crypto.randomUUID(),
        }) as unknown as WalletSetupResponse;
